...
async function run() {
  ...
  for (const oldId of ALICE_OLD_IDS) {
    ...
    if (r.modifiedCount) console.log(`  Alice "${oldId}" → "${ALICE_NEW_ID}": ${r.modifiedCount} docs`);
  }
  ...
  for (const oldId of BOB_OLD_IDS) {
    ...
    if (r.modifiedCount) console.log(`  Bob "${oldId}" → "${BOB_NEW_ID}": ${r.modifiedCount} docs`);
  }
  ...
  if (r.modifiedCount) console.log(`  stamped missing createdByUserId as Alice: ${r.modifiedCount} docs`);

  // Rebuild index
  try { await debts.dropIndex('name_1'); } catch {}
  try { await debts.dropIndex('name_1_user_id_1'); } catch {}
  ...
  if (r.modifiedCount) console.log(`  stamped missing: ${r.modifiedCount} docs`);

  try { await recurring.dropIndex('name_1'); } catch {}
  try { await recurring.dropIndex('name_1_user_id_1'); } catch {}
  await recurring.createIndex({ name: 1, createdByUserId: 1 }, { unique: true });
  console.log('  index (name, createdByUserId) created');
  ...
  for (const oldId of ALICE_OLD_IDS) {
    ...
    if (r.modifiedCount) console.log(`  Alice ownerId "${oldId}" → new ID: ${r.modifiedCount} docs`);
    ...
    if (r.modifiedCount) console.log(`  Alice authorizedUsers "${oldId}" → new ID: ${r.modifiedCount} docs`);
  }
  for (const oldId of BOB_OLD_IDS) {
    r = await lists.updateMany(
      { ownerId: oldId },
    ...
    if (r.modifiedCount) console.log(`  Bob ownerId "${oldId}" → new ID: ${r.modifiedCount} docs`);

    r = await lists.updateMany(
      { authorizedUsers: oldId },
      { $set: { 'authorizedUsers.$': BOB_NEW_ID } }
    );
    if (r.modifiedCount) console.log(`  Bob authorizedUsers "${oldId}" → new ID: ${r.modifiedCount} docs`);
  }

  // Now copy ownerId → createdByUserId for all lists missing the field
  const listsWithoutField = await lists.find({ createdByUserId: { $exists: false } }).toArray();
  let stamped = 0;
  ...
  if (stamped) console.log(`  copied ownerId → createdByUserId: ${stamped} docs`);
